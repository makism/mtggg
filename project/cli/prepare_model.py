...
class PrepModel(FlowSpec):
    ...
    def tokenize(self):
        ...
        data_list = [{'number': card['number'], 'text': card[field_text]} for card in self.cards if field_text in card if card[field_text] is not None]
    ...
    def embed(self):
        ...
        rng = np.random.RandomState(0)
        self.embedded_docvecs = umap.UMAP(n_neighbors=5, metric='cosine', random_state=rng, transform_seed=rng).fit_transform(self.docvecs)
